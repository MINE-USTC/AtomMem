...
from atommem_core.event_level_pipeline import (
    EVENT_LEVEL_TOP_K,
    EVENT_SCORE_EMBEDDING_WEIGHT,
    EVENT_SCORE_KEYWORD_WEIGHT,
    REPRESENTATIVE_FACTS_PER_EVENT,
...
class FactSeededEventLevelEventManager(EventLevelEventManager):
    ...
    def _retrieve_top_event_candidates(
        ...
        event_id_to_trigger_facts: Dict[str, List[Dict[str, Any]]] = {}
            ...
            if fact_event_ids:
                for event_id in fact_event_ids:
                    if event_id not in event_lookup:
                        continue
                    event_id_to_trigger_facts.setdefault(event_id, []).append(
                        self._format_trigger_fact(fact)
                    )
            else:
        ...
        for event_id, trigger_facts in event_id_to_trigger_facts.items():
            event = event_lookup.get(event_id)
            if not event:
                continue

            candidate = self._build_existing_event_candidate(event, new_fact, all_facts)
            if not candidate:
                continue

            candidate["event_summary_score"] = candidate["score"]
            candidate["score"] = max(
                trigger.get("score", 0.0)
                for trigger in trigger_facts
            )
            candidate["trigger_facts"] = trigger_facts
            candidate["representative_facts"] = self._merge_trigger_and_representative_facts(
                trigger_facts,
                candidate.get("representative_facts", []),
            )
    ...
    def _retrieve_top_similar_facts(
        ...
        for fact in all_facts:
            ...
            score = (
                EVENT_SCORE_EMBEDDING_WEIGHT * emb_sim
                + EVENT_SCORE_KEYWORD_WEIGHT * kw_sim
            )
        ...
        candidates.sort(key=lambda item: item["score"], reverse=True)
        return candidates[:EVENT_LEVEL_TOP_K]

    @staticmethod
    def _format_trigger_fact(fact: Dict[str, Any]) -> Dict[str, Any]:
        return {
            "fact_id": fact.get("fact_id"),
            "fact": fact.get("fact", ""),
            "score": round(fact.get("score", 0.0), 4),
        }

    @staticmethod
    def _merge_trigger_and_representative_facts(
        ...
        for fact in trigger_facts + representative_facts:
            fact_id = fact.get("fact_id")
        ...
        return merged[:REPRESENTATIVE_FACTS_PER_EVENT]
