...
EVENT_LEVEL_TOP_K = 10
EVENT_SCORE_EMBEDDING_WEIGHT = 0.7
EVENT_SCORE_KEYWORD_WEIGHT = 0.3
REPRESENTATIVE_FACTS_PER_EVENT = 3
...
class EventLevelEventManager(EventManager):
    ...
    def _build_existing_event_candidate(
        ...
        return {
            ...
            "representative_facts": self._get_representative_event_facts(new_fact, event, all_facts),
        }
    ...
    def _get_representative_event_facts(
        self,
        new_fact: Dict[str, Any],
        event: Dict[str, Any],
        all_facts: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        """Return a few member facts most similar to the new fact for LLM context."""
        fact_lookup = {fact.get("fact_id"): fact for fact in all_facts}
        member_facts = [
            fact_lookup[fact_id]
        ...
        return [
            ...
            for item in scored_facts[:REPRESENTATIVE_FACTS_PER_EVENT]
        ]
